...
ALTER TABLE `gs_pump` ADD COLUMN `project_num` varchar(50) DEFAULT NULL COMMENT '项目号';
ALTER TABLE `gs_pump` ADD COLUMN `monomer_num` varchar(50) DEFAULT NULL COMMENT '单体号';
```
...
```
ALTER TABLE `gs_pipeclass` ADD COLUMN `created_at` int DEFAULT NULL COMMENT '创建时间';
ALTER TABLE `gs_pipeclass` ADD COLUMN `updated_at` int DEFAULT NULL COMMENT '更新时间';
ALTER TABLE `gs_pipeclass` ADD COLUMN `deleted_at` int DEFAULT NULL COMMENT '删除时间';
...
ALTER TABLE `gs_pipeclass` MODIFY COLUMN `iscombustible` tinyint DEFAULT NULL COMMENT '是否可燃，0 不可燃，1 可燃';
```