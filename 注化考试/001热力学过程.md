...
另一个角度，热力学研究的领域是能量的相互转化，涉及到化工过程，就是热和功的转化关系；
...
摘自《ITCET》
6、热力学第一定律的概念；
能量守恒；它是应用于系统和环境2者结合起来的整体的，即系统的能量变化加上环境的能量变化为0；
7、封闭系统的能量守恒；
Δ(Energy of system) = Q+W
ΔUt=Q+W
可以说是最重要的公式，它揭示了我们最终需要的热Q、功W与可测量的参数PVT之前的关系；PVT可以计算内能的变化值，通过内能的变化值计算我们想要的热和功；
另一个角度来看；从状态1到状态2，可以有很多过程(路径)来实现，每一个路径的Q和W是不同的，但Q+W肯定是一样，也就是内能变化值是不变的，也说明了内能是一个状态变量，一个状态对应着一个特定的值；同时利用这一点，算一个实际过程的热力学性质变化时就可以用等效的思维，可以把一个实际的过程用另一个虚拟的路线代替算起功和热，只有初、终2个状态点与实际的一样；
8、热力学中平衡的概念；
不仅没有变化，还不能有变化的趋势；
